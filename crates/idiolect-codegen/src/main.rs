...
fn write_generated(out_dir: &Path, files: &[EmittedFile]) -> Result<()> {
    ...
    // Walk the existing tree once, collecting every `@generated` file
    // and every directory under `out_dir`. Anything not in the new
    // file set gets pruned; empty directories are removed bottom-up.
    let keep: std::collections::HashSet<String> = files.iter().map(|f| f.path.clone()).collect();
    ...
    walk(out_dir, &mut existing_files, &mut existing_dirs)?;

    for victim in existing_files {
        let rel = victim
            .strip_prefix(out_dir)
            .ok()
            .map(|p| p.to_string_lossy().replace('\\', "/"));
        if rel.as_deref().is_some_and(|r| keep.contains(r)) {
            continue;
        }
    ...
    for d in existing_dirs.into_iter().rev() {
        if fs::read_dir(&d).is_ok_and(|mut it| it.next().is_none()) {
...
fn check_drift(out_dir: &Path, files: &[EmittedFile]) -> Result<bool> {
    ...
    if out_dir.is_dir() {
        ...
        // Recursive walk: with the lexicon-tree layout the generated
        // tree is nested, so a top-level scan would miss stale files
        // like `dev/idiolect/<deleted>.rs`. Mirror the recursive walk
        // `write_generated` already uses.
        fn walk_files(dir: &Path, files: &mut Vec<std::path::PathBuf>) -> Result<()> {
            for entry in fs::read_dir(dir).with_context(|| format!("reading {}", dir.display()))? {
                let entry = entry?;
                let path = entry.path();
                let ft = entry.file_type()?;
        ...
        walk_files(out_dir, &mut existing)?;
