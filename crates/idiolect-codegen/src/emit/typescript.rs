    ...
    fn emit(
        ...
        for doc in docs {
            out.push(EmittedFile {
                path: format!("{}.ts", module_name_for_nsid(&doc.nsid)),
...
fn ts_ref_type(target: &RefTarget, current_nsid: &str) -> TsType {
    if target.nsid == current_nsid {
        ...
        TsType::Ref {
            name: pascal_case(&target.def_name),
            import_from: Some(module_name_for_nsid(&target.nsid)),
        }
    }
}
...
fn render_index_ts(docs: &[LexiconDoc]) -> String {
    let mut out = String::new();
    out.push_str("// @generated by idiolect-codegen. do not edit.\n\n");
    out.push_str(
    ...
    modules.extend(["examples".to_owned(), "records".to_owned()]);
    modules.sort();
    for m in &modules {
        let _ = writeln!(out, "export * from \"./{m}\";");
    }
    out
}
...
fn render_records_ts(docs: &[LexiconDoc]) -> String {
    ...
    for r in &records {
        ...
        let _ = writeln!(out, "import type {{ {ty} }} from \"./{kind}\";");
    }
    out.push('\n');
