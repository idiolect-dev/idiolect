...
const fn kind_tag(record: &AnyRecord) -> &'static str {
    match record {
        ...
        AnyRecord::ChangeProposal(_) => "changeProposal",
        AnyRecord::CommunityRelease(_) => "communityRelease",
        AnyRecord::Dialect(_) => "dialect",
        AnyRecord::Federation(_) => "federation",
        AnyRecord::MigrationRun(_) => "migrationRun",
...
fn serialize_record_body(record: &AnyRecord) -> OrchestratorResult<String> {
    let value = match record {
        ...
        AnyRecord::ChangeProposal(r) => serde_json::to_string(r),
        AnyRecord::CommunityRelease(r) => serde_json::to_string(r),
        AnyRecord::Dialect(r) => serde_json::to_string(r),
        AnyRecord::Federation(r) => serde_json::to_string(r),
        AnyRecord::MigrationRun(r) => serde_json::to_string(r),
...
fn deserialize_record_body(kind: &str, body: &str) -> OrchestratorResult<AnyRecord> {
    ...
    Ok(match kind {
        ...
        "changeProposal" => AnyRecord::ChangeProposal(from(body)?),
        "communityRelease" => AnyRecord::CommunityRelease(from(body)?),
        "dialect" => AnyRecord::Dialect(from(body)?),
        "federation" => AnyRecord::Federation(from(body)?),
        "migrationRun" => AnyRecord::MigrationRun(from(body)?),
